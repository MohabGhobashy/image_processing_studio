...
void Threshold(Mat& input_image, Mat& output_image,int threshold )
{
    ...
    for (int j = 0; j < input_image.cols; j++)
    {
        if (input_image.at<uchar>(i, j) > threshold) { output_image.at<uchar>(i, j) = 0;}

        else { output_image.at<uchar>(i, j) = 255; }
    }
}
 ...
 * first it selects the neighbouring 8 pixels for each pixel in the picture, then sort them and inject the median in the current pix
*/
void localThreshold( Mat& input, Mat& output, int blockSize, double C)
{
    ...
    for (int x = 0; x < input.cols; x++)
    {
        ...
        int threshold = (int)round(mean(pixelValues)[0] - C);

        // Apply the threshold to the pixel
        if (input.at<uchar>(y, x) >= threshold) { output.at<uchar>(y, x) = 255; }

        else { output.at<uchar>(y, x) = 0; }
    }
}
